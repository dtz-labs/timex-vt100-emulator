 ...
 *
 * RUN COALESCING. HIRES_THIRDS_OFFSET() decomposes a pixel row as
 *
 ...
 * calls per display file (each preceded by two HIRES_THIRDS_OFFSET
 * evaluations) to ~24 memmove calls plus the boundary memcpys, which is
 * where scroll_vram's cost actually lived. Recommendation 5 of
    ...
    for (scanline = 0; scanline < 8u; ++scanline) {
        ...
        while (row > top) {
            ...
            if (run == 0u) {
                ...
                dst = (u8 *)(uintptr_t)(base + HIRES_THIRDS_OFFSET((u8)((row << 3) + scanline)));
                src = (const u8 *)(uintptr_t)(base + HIRES_THIRDS_OFFSET((u8)(((row - 1u) << 3) + scanline)));
                memcpy(dst, src, 32u);
                --row;
                continue;
            }
             ...
             * (LDDR). */
            dst = (u8 *)(uintptr_t)(base + HIRES_THIRDS_OFFSET((u8)(((row - run + 1u) << 3) + scanline)));
            src = (const u8 *)(uintptr_t)(base + HIRES_THIRDS_OFFSET((u8)(((row - run) << 3) + scanline)));
            memmove(dst, src, (u16)run * 32u);
            row = (u8)(row - run);
        }
    }
