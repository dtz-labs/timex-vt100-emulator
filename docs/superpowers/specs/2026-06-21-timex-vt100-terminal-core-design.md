...
2. **M2 — pure core:** ✅ **DONE (2026-06-22).** `screen` + `vtparse` host-TDD over the §6 subset (red/green/refactor). `screen.c`: init/putc (with VT-100 deferred wrap)/cup/scroll/cr/lf/ri/erase line+display/IL/DL/ICH/DCH/SGR set_attr/save+restore cursor/modes (DECAWM, DECTCEM)/DECSTBM. `vtparse.c`: GROUND printables + C0 (BS/HT/CR/LF/FF/SI/SO), ESC finals (RIS/IND/RI/NEL/DECSC/DECRC), CSI cursor moves (CUU/CUD/CUF/CUB, CUP/HVP), erase/edit (ED/EL/IL/DL/ICH/DCH), SGR, DECSTBM, private modes (?7/?25), device queries (DSR 6n, primary DA → vt->out), and G0/G1 charset + DEC line-drawing (graphics bytes → high-bit font page `ch|0x80`). **3213 screen + 89 vtparse host checks**; both modules also compile clean under z88dk/SDCC `+zx`.
